...
class RAGEvaluator:
    ...
    def _get_sample_municipalities(self, assistant: PECCAssistant) -> List[str]:
        ...
        # try:
        #     # Accède aux données des communes via le document processor
        ...
        # # Communes par défaut si pas d'accès aux données
        # return ["Lausanne", "Genève", "Choulex", "Nyon", "Vevey", "Montreux", "Sion", "Martigny", "Neuchâtel", "Fribourg"]
    ...
    def _get_expected_contexts_for_municipality(self, assistant: PECCAssistant, municipality: str) -> List[str]:
        ...
        # Recherche dans l'index pour identifier les chunks pertinents
        try:
            # Recherche avec différentes requêtes liées à la commune
            search_queries = [
                f"{municipality}",
            ...
            for query in search_queries:
                ...
                for result in results:
                    if result["similarity_score"] > 0.7:  # Seuil de pertinence
    ...
    def _get_real_user_questions(self) -> List[Dict]:
        ...
        # Exemples de questions réelles basées sur votre domaine
        real_questions = [
            {
                ...
                "expected_contexts": [],  # À remplir par recherche
    ...
    def calculate_hit_rate(self, retrieved_contexts: List[str], expected_contexts: List[str]) -> float:
        ...
        if not expected_contexts:
            return 1.0  # Si pas de contexte attendu, considère comme succès
    ...
    def calculate_ndcg(self, retrieved_contexts: List[str], expected_contexts: List[str], k: int = 5) -> float:
        ...
        for i, context in enumerate(retrieved_contexts[:k]):
            if context in expected_contexts:
                dcg += 1.0 / np.log2(i + 2)  # i+2 car log2(1) = 0
    ...
    async def evaluate_retrieval_config(self, 
        ...
        for pair in question_pairs:
            ...
            # Récupère les documents avec la configuration donnée
    ...
    async def _retrieve_with_config(self, assistant: PECCAssistant, query: str, config: Dict) -> List[Dict]:
        ...
        # Analyse la requête pour le contexte
        ...
        # Autres méthodes de recherche seront implémentées plus tard
        return assistant.vector_store.search(query, k=k)
